...
import { srmiGetEmoticon, srmiGetNewsList, srmiGetPostFull, PostFullPost } from "#kazuha.models";
...
export async function srnewsContentBBS(msg: IMessageEx) {
    ...
    const pagesData = await srmiGetNewsList(type);
    ...
    const postFull = await srmiGetPostFull(pagesData.list[page - 1].post.post_id);
    if (!postFull) return;
...
export async function srnewsListBBS(msg: IMessageEx) {
    ...
    const data = await srmiGetNewsList(type, 5);
    if (!data) return;
...
async function mysEmoticon() {
    const emp = new Map();
    const res = await srmiGetEmoticon();
    if (!res) return null;
