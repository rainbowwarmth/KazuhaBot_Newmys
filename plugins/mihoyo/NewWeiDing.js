...
exports.wdnewsContentBBS = wdnewsContentBBS;
...
async function wdnewsContentBBS(msg) {
    ...
    if (msg.content.includes("活动"))
        type = 2;
    const pagesData = await kazuha_1.default.wdmiGetNewsList(type);
    ...
    const postFull = await kazuha_1.default.wdmiGetPostFull(pagesData.list[page - 1].post.post_id);
    if (!postFull)
        return;
    ...
    kazuha_1.default.render({
        ...
        logger_1.default.mark(kazuha_1.default.chalk.blueBright(`[未定事件簿公告] newsContentBBS/NewWeiDing.ts`));
    }).catch((err) => {
        logger_1.default.error(err);
    });
}
async function wdnewsListBBS(msg) {
    var type = 1, typeName = "公告";
    if (msg.content.includes("资讯"))
        type = 3, typeName = "资讯";
    if (msg.content.includes("活动"))
        type = 2, typeName = "活动";
    const data = await kazuha_1.default.wdmiGetNewsList(type, 5);
    if (!data)
        return;
    var datas = data.list;
    if (datas.length == 0) {
    ...
    await kazuha_1.default.render({
        ...
        if (savePath)
            msg.sendMsgEx({ imagePath: savePath });
        logger_1.default.mark(kazuha_1.default.chalk.blueBright(`[未定事件簿公告列表] newListBBS/NewWeiDing.ts`));
    }).catch((err) => {
        logger_1.default.error(err);
    });
}
async function wdchangePushTask(msg) {
    if (msg.messageType != "GUILD")
        return true;
    const value = msg.content.includes("开启") ? true : false;
    await global_1.redis.hSet("config:wdnewsPush", parseInt(msg.channel_id), `${value}`).then((v) => {
        if (value)
            return msg.sendMsgEx({
                content: `未定事件簿米游社公告推送已开启` + `\n每1分钟自动检测一次是否存在新更新公告` + `\n如有更新自动发送公告内容至此。`
            });
        else {
            return msg.sendMsgEx({ content: `未定事件簿米游社公告推送已关闭` });
        }
    }).catch(err => {
        logger_1.default.error(err);
    });
...
async function wdtaskPushNews() {
    ...
    if (!msgId)
        return;
    const sendChannels = [];
    const _newsPushChannels = await global_1.redis.hGetAll("config:wdnewsPush").catch(err => { logger_1.default.error(err); });
    ...
    const pagesData = [{ type: "公告", list: (await kazuha_1.default.wdmiGetNewsList(1))?.list }, { type: "资讯", list: (await kazuha_1.default.wdmiGetNewsList(3))?.list }];
    const postIds = [];
    ...
    for (const postId of postIds) {
        const postFull = await kazuha_1.default.wdmiGetPostFull(postId);
        if (!postFull)
            return;
        const data = await kazuha_1.default.detalData(postFull.post);
        //log.debug(data);
        await kazuha_1.default.render({
            ...
            if (savePath) {
                ...
                logger_1.default.mark(kazuha_1.default.chalk.blueBright(`[未定事件簿公告推送] taskPushNews/NewWeiDing.ts`));
...
//# sourceMappingURL=NewWeiDing.js.map